...
import random
from ..utils import bank,MoneySelector,uis,game_template
import asyncio
class GuessNumber(game_template.Template):
    ...
    async def start_game(self, range: int,interaction: discord.Interaction):
        ...
        if self.bet > self.account.money:
            await self.interaction.response.send_message("You don't have enough money to bet that much!",ephemeral=True)
            return
        
        await self.interaction.edit_original_response(content=f"Guess a number between 1 and {range}")
        ...
        if modal.answer is None:
            return await self.interaction.followup.send("You didn't provide an answer",ephemeral=True)
        ...
        await self.interaction.response.send_message(f"Are you sure you want to guess {modal.answer}?",view=confirmation)
        ...
        if number == answer:
            await self.interaction.edit_original_message(content=f"You won! You got the number right!\nPrize: {self.bet * mapped[range]}")
            self.account.money += self.bet * mapped[range]
        elif number > answer:
            await self.interaction.edit_original_message(content=f"Your answer was too low\nThe number was {number}\nPrize: -{self.bet * mapped[range]}")
            self.account.money -= self.bet * mapped[range]
        elif number < answer:
            await self.interaction.edit_original_message(content=f"Your answer was too high\nThe number was {number}\nPrize: -{self.bet * mapped[range]}")
            self.account.money -= self.bet * mapped[range]

        
    async def actually_starting_the_game_with_rapid_pace_on_god(self):
        ui = uis.Multiple_Buttons()
        ui.Add_Button("Easy (10 range, 0.5 multiplier)",self.easy,style=discord.ButtonStyle.primary)
        ui.Add_Button("Medium (25 range, 0.85 multiplier)",self.medium,style=discord.ButtonStyle.primary)
        ui.Add_Button("Hard (50 range, 1 multiplier)",self.hard,style=discord.ButtonStyle.primary)
        ui.Add_Button("Impossible 💀 (200 range, 2 multiplier)",self.impossible,style=discord.ButtonStyle.primary)
        await self.interaction.edit_original_response(content="Please select a difficulty",view=ui)
        await ui.wait()
    ...
    async def start(self, interaction: discord.Interaction):
        ...
        self.interaction = interaction
        self.account = await bank.Player_Status.get_by_id(self.interaction.user.id)
        await MoneySelector.MoneySelector(self.interaction,self.callback_money).get_money()
