...
class twentyOne(game_template.Template):
    ...
    async def PullOut(self, Interaction: discord.Interaction, label: str):
        ...
        moneyReward = self.betValue * moneyMulti
        self.account.money += int(moneyReward)
        
        await Interaction.response.send_message(f"You pulled out at score: {self.score}", ephemeral=True)
        await self.Interaction.edit_original_response(content=f"Well done you earned {moneyReward} (Multiplayer: {moneyMulti})", view=None)
    ...
    async def rollDice(self, Interaction: discord.Interaction, label: str):
        ...
        self.score += diceRole
        await Interaction.response.send_message(f"You rolled a {diceRole}.", ephemeral=True)
        await self.show_ui()
    
    async def show_ui(self):
        logger.debug("Showing ui")
        if self.score > 21:  # Only stop if they go over, don't warn them for anything else.
            return await self.Interaction.edit_original_response(content=f"You went over the limit and lost everything.", view=None)
        
        await self.Interaction.edit_original_response(content=f"Current value: {self.score}. What do you do?", view=self.view)
