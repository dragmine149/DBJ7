...
from src.utils import uis, bank, game_template
...
class HigherOrLower(game_template.Template):
    ...
    async def Finish(self):
        ...
        await self.Interaction.edit_original_response(content=f"You got a multiplier of {str(moneyMultiplier)[0:5]}x.\nFor a reward of {moneyReward}\n\nGame Information", embed=self.GenerateEmbed(), view=None)
    ...
    def GenerateEmbed(self) -> discord.Embed:
        ...
        self.embed.set_field_at(
            0, name=self.embed.fields[0].name, 
            value=f"{cc[1]} of {cc[0]}")

        self.embed.set_field_at(
            1, name=self.embed.fields[1].name, 
            value=f"{str(round((self.currentCard + 1) / 52, 4) * 100)[0:5]}%")

        self.embed.set_field_at(
            2, name=self.embed.fields[2].name,
            value=self.correct
        )
    ...
    async def GuessCallback(self, Interaction: discord.Interaction, label: str):
        ...
        if Interaction.user.id != self.user.id:
            return await Interaction.response.send_message(
                "You can not choose an option for this user!",
                ephemeral=True
            )
        ...
        await Interaction.response.send_message(content=f"You were {correctmsg}, the new card is {highermsg} than the old card. Left: {52 - (self.currentCard + 1)}")
    ...
    async def HigherLower(self):
        ...
        await self.Interaction.edit_original_response(content="", view=self.view, embed=self.GenerateEmbed())
    ...
    async def start(self, Interaction: discord.Interaction):
        ...
        self.embed.set_author(name=self.user.display_name,
