...
class game_loader(commands.Cog):
        ...
        for game in self.games:
            if type(game).__name__ == values[0]:
                try:
                    ...
                    await Interaction.response.send_message("Failed to start game! (game code not coded correctly)")
                    self.logger.error(f"Failed to start {values[0]}! Missing start function")
    ...
    async def playgame(self, ctx: commands.Context):
        ...
        for game in self.games:
            ...
            gameOptions.append(discord.SelectOption(label=type(game).__name__, description=desc, emoji=emoji))
        
        view = uis.DropdownView(callback=self.game_select, placeholder="Select game to play", options=gameOptions)
