...
class Multiplayer:
    ...
    async def Ask(self):
        ...
        try:
            await self.Interaction.response.send_message(f"Do you want to play with other people?", view=multiplayer)
        except discord.errors.InteractionResponded:
            await self.Interaction.edit_original_response(content=f"Do you want to play with other people?", view=multiplayer)
    ...
    async def AcceptInvitation(self, Interaction: discord.Interaction, label: str):
        ...
        if len(self.data) < self.limit:
            ...
            await Interaction.response.send_message("Joined game!\n\nPlease choose amount of money to bet", ephemeral=True)
            ms = MoneySelector(Interaction, self.MoneyCallback, userOnly=True)
            await ms.get_money()
        else:
            # Notifies user
            await Interaction.response.send_message("Sorry but this game is full currently.", ephemeral=True)
    ...
    async def Invite(self, Interaction: discord.Interaction, label: str):
        ...
        await Interaction.response.send_message(f"Press the button to play with: {self.owner.mention} in {self.game}", view=self.buttonView)
        await self.callback()
