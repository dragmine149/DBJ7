...
class Accounting(commands.Cog):
    ...
    async def on_member_join(self, member: discord.Member) -> None:
        try:
            exists = True if await FileHandler().ReadFile(f"accounting/{member.id}.json") else True
        except FileNotFoundError:
            exists = False
            
        if not exists:
            await FileHandler().SaveFile(f"accounting/{member.id}.json", {"money": 20000}) # 20k for joining
            await member.send(
                embed=discord.Embed(
    ...
    async def find_member_not_exists_in_db(self) -> None:
        for member in self.bot.get_all_members():
            try:
                exists = True if FileHandler().ReadFile(f"accounting/{member.id}.json") else True
            except FileNotFoundError:
                exists = False
                
            if not exists:
                FileHandler().SaveFile(f"accounting/{member.id}.json", {"money": 20000},)
